...
export default class IMOperator {
    ...
    constructor(page, opts) {
      ...
      this._myHeadUrl = wx.getStorageSync('userInfo').avatarUrl ? wx.getStorageSync('userInfo').avatarUrl : app.globalData.userInfo.avatarUrl;
        ...
        getApp().getIMHandler().setOnReceiveMessageListener({
            listener: (msg) => {
              ...
              if (msg.type === 'get-history'){
                var length = msg.conversations.length;
                var userid = wx.getStorageSync('userid');
                if (!userid) {
                  userid = app.globalData.userid
                }
                for (var i = 0; i < length;i++) {
                   var tempmsg = msg.conversations[i];
                //  console.log(tempmsg);
                  msg.isMy = tempmsg.msgUserId === userid;
                  if (tempmsg.type == 'voice') {
                    var temp = { type: tempmsg.type, content: tempmsg.content, isMy: msg.isMy, duration: tempmsg.duration }
                  } else {
                    var temp = { type: tempmsg.type, content: tempmsg.content, isMy: msg.isMy}
                  }
                  const item = this.createNormalChatItem(temp);
                // console.log(item);
                  this._latestTImestamp = item.timestamp;
                  item.chattype = 'history'
                  item.index = i+1;
                  item.length = length;
                  if (msg.loadstatus==1){
                  ...
                  var data = msg.data; 
                  if (data.type == 'voice'){
                    var temp = { type: data.type, content: data.content, isMy: false, duration: data.duration }                   
                  }else{
                    var temp = { type: data.type, content: data.content, isMy: false, }
                  }
                  const item = this.createNormalChatItem(temp);
                  //console.log(item);
